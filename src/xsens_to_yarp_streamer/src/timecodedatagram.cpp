...
/*! Constructor */
TimeCodeDatagram::TimeCodeDatagram(yarp::os::BufferedPort<yarp::os::Bottle> &port)
	: Datagram(port)
{
	setType(SPTimeCode);
}
...
void TimeCodeDatagram::publishData()
{
	yarp::os::Bottle& bottle = port.prepare();
	bottle.clear();

	bottle.addInt(frameTime());
	bottle.addInt(m_hour);
	bottle.addInt(m_minute);
	bottle.addInt(m_second);
	bottle.addInt(m_nano);
	port.write();
}
