...
/*! Constructor */
EulerDatagram::EulerDatagram(yarp::os::BufferedPort<yarp::os::Bottle> &port)
	: Datagram(port)
{
	setType(SPPoseEuler);
}
...
void EulerDatagram::publishData()
{
	...
	for (int i = 0; i < m_data.size(); i++)
	{
		...
		bottle.addDouble(m_data.at(i).rotation[0]);
		bottle.addDouble(m_data.at(i).rotation[1]);
		bottle.addDouble(m_data.at(i).rotation[2]);
