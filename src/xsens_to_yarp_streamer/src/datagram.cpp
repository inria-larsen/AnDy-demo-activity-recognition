...
Datagram::Datagram(yarp::os::BufferedPort<yarp::os::Bottle> &port) :
		port(port),
		m_header("MXTP00"),
		m_avatarId(0),
		m_dgramCounter(0x80),
...
void Datagram::publishData()
{
	std::cerr << "Error: don't know how to stream " << decode(static_cast<StreamingProtocol>(messageType()))  << std::endl;
}
