...
/*! Constructor */
PositionDatagram::PositionDatagram(yarp::os::BufferedPort<yarp::os::Bottle> &port)
	: Datagram(port)
{
...
void PositionDatagram::publishData()
{
	...
	for (int i = 0; i < m_data.size(); i++)
	{
		...
		bottle.addDouble(m_data.at(i).pointPos[0]);
		bottle.addDouble(m_data.at(i).pointPos[1]);
		bottle.addDouble(m_data.at(i).pointPos[2]);
	}
	port.write();
}
