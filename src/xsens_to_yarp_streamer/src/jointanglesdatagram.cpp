...
/*! Constructor */
JointAnglesDatagram::JointAnglesDatagram(yarp::os::BufferedPort<yarp::os::Bottle> &port)
	: Datagram(port)
{
...
void JointAnglesDatagram::publishData()
{
	...
	for (int i = 0; i < m_data.size(); i++)
	{
	//bottle.addInt(m_data.at(i).parent);
	//bottle.addInt(m_data.at(i).child);
